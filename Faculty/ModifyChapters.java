...
public class ModifyChapters {
    ...
    public static void modifyChapters(Scanner scanner,String courseId) {
        ...
        String textbookId = AddNewChapter.getTextbookId(courseId);
    ...
    public static void deleteChapter(Scanner scanner, String chapterId) {
        ...
        try {
            ...
            if (conn != null) {
                String query = "DELETE FROM Chapters WHERE chapter_id = ? and (createdby= ? or createdby = ?)";
                pstmt = conn.prepareStatement(query);
                pstmt.setString(1, chapterId);
                ...
                if (rowsDeleted > 0) {
                    ...
                    System.out.println("Failed to delete chapter as it is created by admin or chapter doesnot exists");
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
